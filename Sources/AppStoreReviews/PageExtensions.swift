...
extension Page {
    ...
    /// * https://itunes.apple.com/fr/rss/customerreviews/page=2/id=1/sortby=mostrecent/xml?l=en&urlDesc=/customerreviews/page=2/id=1/sortby=mostrecent/json
    public init?(_ url: URL) {
        guard let components = URLComponents(url: url, resolvingAgainstBaseURL: true) else {
            return nil
        }
        do {
            if let urlDesc = components.queryItems?.first(where: { item in item.name == "urlDesc" })?.value,
               let page = Self.pageWithPath(path: components.path, urlDesc: urlDesc) {
...
private extension Page {
    static func pageWithPath(path: String, urlDesc: String) -> Page? {
        ...
        guard let page = Self.intValue(in: urlDesc, key: .page),
              let appID = Self.intValue(in: urlDesc, key: .id) else {
            return nil
        }
